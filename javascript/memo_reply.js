...
Coca = {
	...
	on_upload_complete: function(result, ctx){
		...
		if (ctx == "my_image_uploader") {
			if( size > 1024 * 500 ) {
				alert("이미지 직접 올리기는 최대 500kb까지 가능합니다.");
				return false;
			}
	...
	on_over_filesize: function(filename, maxsize, ctx){
		...
		alert("이미지 용량이 너무 큽니다. 최대 2MB까지 가능합니다.");
	},
 ...
 */
Memo.Message = {
	NO_COMMENTS: "등록된 댓글이 없습니다. 댓글을 등록해 주세요.",
	DENY_PERMISSION: "댓글쓰기 권한이 없습니다.",
...
DWRErrorController = {
	...
	handleError: function(errorMessage, exception) {
		if (exception.code == "LOGIN" || exception.code == "LOGIN_SIMPLE" || exception.code.indexOf("스팸") != -1) {
